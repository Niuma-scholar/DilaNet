...
class FasterRCNNTrainer(nn.Module):
    ...
    ''' 
    通过计算边框坐标的预测值，边框的真实坐标，真实的分类标签，边界框回归损失的标准差，计算RPN边界框回归的损失值
    '''
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        for i in range(n):
            ...
            rpn_loc是RPN的边界框坐标预测值，gt_rpn_loc是真实的边界框坐标，gt_rpn_label是真实的分类标签，self.rpn_sigma是用于计算边界框回归损失的标准差。
            通过调用self._fast_rcnn_loc_loss()函数，将这些值传入，即可计算得到RPN边界框回归的损失值。  
            
            使用F.cross_entropy()函数计算损失。rpn_score是RPN的分类得分，gt_rpn_label是真实的分类标签。该函数会计算交叉熵损失，用于衡量预测的分类概率与真实标签之间的差异。
            '''
        ...
        for i in range(n):
            ...
            roi_score是模型预测的ROI分类得分，它的形状是(batch_size, num_classes)，表示每个ROI对应于各个类别的得分。
            gt_roi_label是真实的ROI分类标签，它的形状是(batch_size,)，表示每个ROI的真实类别标签。
            nn.CrossEntropyLoss()是交叉熵损失函数，用于计算多类别分类问题的损失。
            整个代码的含义是，使用交叉熵损失函数计算模型预测的ROI分类得分和真实ROI分类标签之间的损失
            '''
            # -------------------------------------------------- #
